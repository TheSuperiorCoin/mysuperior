# Open Superior - a fully open sourced implementation of MySuperior backend

In this example [restbed](https://github.com/Corvusoft/restbed/) is used to
demonstrate how to provide Superior related JSON REST service. For this purpose,
 a service called Open Superior was developed.
...
to MySuperior. They include:

 - google analytics, cloudflare, images and flash were removed.
 - frontend's html and javascript upgrades and cleanups.
 - transaction service fee was set to zero (same as in MySuperior).
 - the wallets generated use 25 word mnemonics, fully compatible with official superior wallets
(13 word mnemonics generated by MySuperior work as usual though).
 ...
 - private tx key given to a user which can be used to prove sending sup to a recipient.
...
The Open Superior consists of four components that need to be setup for it to work:
 ...
 - Frontend - it is virtually same as that of MySuperior, except before mentioned differences.
  It consists of HTML, CSS, and JavaScript.
 - Superior daemon - daemon must be running and fully sync, as this is
 where all transaction data is fetched from and used. Daemon also commits txs
 from the Open Superior into the Superior network.
 - Backend - fully written in C++. It uses [restbed](https://github.com/Corvusoft/restbed/) to serve JSON REST to the frontend
 and [mysql++](http://www.tangentsoft.net/mysql++/) to interface the database. It also accesses Superior blockchain and "talks"
 with Superior deamon.
...
Below are example and basic instructions on how to setup up and run Open Superior on Ubuntu 16.04.
...
#### Compilation of the Open Superior (don't run it yet)

Once Superior was downloaded and compiled, we can download Open Superior and compile it.
...
work without database, setup frontend, and synced and running superior blockchain.
...
Download `opensuperior.sql` provided and setup the `opensuperior` database. `opensuperior.sql` script will
drop current `opensuperior` if exist. So don't run it, if you have already some important information
in the `opensuperior` database.
...
Before running `opensuperior`:
 ...
 - make sure superior daemon is running and fully sync. If using testnet or stagenet networks, use superior daemon
