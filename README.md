# Open Superior - a fully open sourced implementation of MySuperiorCoin backend

In this example [restbed](https://github.com/Corvusoft/restbed/) is used to
demonstrate how to provide SuperiorCoin related JSON REST service. For this purpose, 
 a service called Open SuperiorCoin was developed.
...
to MySuperiorCoin. They include:
 ...
 - the wallets generated use 25 word mnemonics, fully compatible with official SuperiorCoin wallets
(13 word mnemonics generated by MySuperiorCoin work as usual though).
...
This is Open SuperiorCoin running on testnet network. You can use it to play around with it. 
...
If you want some testnet SuperiorCoin, please make issue with your testnet address that you can
obtained from Open SuperiorCoin.  

## Screenshot

![Open SuperiorCoin](https://raw.githubusercontent.com/SuperiorCoinexamples/openSuperiorCoin/master/screenshot/screen1.png)


## Host it yourself

The Open SuperiorCoin consists of four components that need to be setup for it to work:
 ...
 - Frontend - it is virtually same as that of MySuperiorCoin, except before mentioned differences.
  It consists of HTML, CSS, and JavaScript.
 - SuperiorCoin daemon - daemon must be running and fully sync, as this is 
 where all transaction data is fetched from and used. Daemon also commits txs 
 from the Open SuperiorCoin into the SuperiorCoin network.
 - Backend - fully written in C++. It uses [restbed](https://github.com/Corvusoft/restbed/) to serve JSON REST to the frontend 
 and [mysql++](http://www.tangentsoft.net/mysql++/) to interface the database. It also accesses SuperiorCoin blockchain and "talks"
 with SuperiorCoin deamon.
...
<<<<<<< HEAD
Open SuperiorCoin is not as fast as MySuperiorCoin. This is because it is basic, easy to understand
...
=======
Open SuperiorCoin is not as fast as MySuperiorCoin.
...
Below are example and basic instructions on how to setup up and run Open SuperiorCoin on Ubuntu 16.04. 
For other Linux operating systems, the instructions are analogical. 


#### SuperiorCoin download and compilation

Download and compile recent SuperiorCoin into your home folder:

```bash
# first install SuperiorCoin dependecines
sudo apt update
...
# get latest development version of SuperiorCoin. SuperiorCoin changes so fast
# that if something breaks during compilation of open SuperiorCoin, please make an issue
git clone https://github.com/SuperiorCoin-project/SuperiorCoin
...
#### Compilation of the Open SuperiorCoin (don't run it yet)

Once SuperiorCoin was downloaded and compiled, we can download Open SuperiorCoin and compile it. 
...
work without database, setup frontend, and synced and running SuperiorCoin blockchain.

```bash
# need mysql++ libraries 
sudo apt install libmysql++-dev
 
# go to home folder if still in ~/SuperiorCoin
cd ~

git clone https://github.com/SuperiorCoinexamples/openSuperiorCoin.git
...
# altearnatively can use cmake -DSuperiorCoin_DIR=/path/to/SuperiorCoin_folder .. 
# if SuperiorCoin is not in ~/SuperiorCoin
...
Download `openSuperiorCoin.sql` provided and setup the `openSuperiorCoin` database. `openSuperiorCoin.sql` script will
drop current `openSuperiorCoin` if exist. So don't run it, if you have already some important information
in the `openSuperiorCoin` database.

Assuming we are still in `build` folder:

```bash
# apply it to mysql
mysql -p -u root < ../sql/openSuperiorCoin.sql
```
...
```bash
sudo mkdir /var/www/html/openSuperiorCoin
sudo cp -rvf ../html/* /var/www/html/openSuperiorCoin/
```

Setup document root in `lighttpd.conf` into openSuperiorCoin folder
...
```bash
server.document-root    = "/var/www/html/openSuperiorCoin"
```
...
Before running `openSuperiorCoin`: 

 - make sure you have `Downloads` folder in your home directory. 
 Time library used in Open SuperiorCoin stores there time zone offsets database that it uses.
 ...
 - make sure SuperiorCoin daemon is running and fully sync. If using testnet network, use daemon
 with testnet flag!
...
```bash
./openSuperiorCoin -t -c /path/to/config.json
```
...
query the testnet open SuperiorCoin instance (assuming its up and working well).
...
```json
{
  ...
  "SuperiorCoin_version_full": "0.10.3.1-125f823"
}
```
...
Other examples can be found on  [github](https://github.com/SuperiorCoinexamples?tab=repositories).
